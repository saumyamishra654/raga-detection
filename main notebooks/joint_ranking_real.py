...
def main():
    ...
    for _, row in gt_df.iterrows():
        ...
        # Process Accompaniment (Unweighted for Salience to match notebook?)
        # Notebook uses: H_acc, _ = np.histogram(pitch_classes_acc, bins=12, range=(0, 12))
        # It does NOT use weights for H_acc.
        ...
        if freqs_a.size:
            ...
            H_acc, _ = np.histogram(pc_a, bins=12, range=(0, 12)) # Unweighted!
            max_val = H_acc.max()
            acc_dist = (H_acc / max_val) if max_val > 0 else np.zeros(12)
        else:
            acc_dist = np.zeros(12)
            
        # Determine Allowed Tonics (Notebook Logic)
        ...
        # 1. Correct Pair
        feats = calculate_features(melody_dist, acc_dist, raga_mask, gt_tonic, detected_peak_count)
        if feats:
            X.append(feats)
            y.append(1)
            old_score = sum(OLD_WEIGHTS[k] * v for k, v in zip(OLD_WEIGHTS.keys(), feats))
        ...
        # 2. Negatives
        # Iterate over unique masks to avoid duplicates
        unique_masks = {}
        for k, v in raga_masks.items():
            # Use tuple of mask as key to identify unique ragas
        ...
        for mask_tuple, cand_raga_name in unique_masks.items():
            ...
            for cand_tonic in range(12):
                ...
                feats = calculate_features(melody_dist, acc_dist, mask, cand_tonic, detected_peak_count)
    ...
    print("\n" + "="*40)
    print("OPTIMIZED WEIGHTS (Standardized)")
    print("="*40)
    feature_names = list(OLD_WEIGHTS.keys())
    coeffs = clf.coef_[0]
    ...
    rank_df['new_score'] = clf.decision_function(feats_scaled)
    ...
    joblib.dump(clf, 'joint_score_model.pkl')
    joblib.dump(scaler, 'joint_score_scaler.pkl')
