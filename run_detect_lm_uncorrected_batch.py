"""Run detect with --use-lm-scoring --lm-skip-correction on all CompMusic recordings.

Option A: uncorrected model + uncorrected scoring. Should be much faster
than per-hypothesis correction (~5s per recording vs ~3min).

Usage: /opt/miniconda3/envs/raga/bin/python run_detect_lm_uncorrected_batch.py
"""
import csv
import subprocess
import sys
...
LM_MODEL = "compmusic_ngram_model_uncorrected.json"
...
total = len(rows)
t0 = time.time()
print(f"Starting batch (Option A - uncorrected): {total} recordings", flush=True)
...
for i, row in enumerate(rows, 1):
    ...
    if result.returncode == 0:
        ...
        print(f"  -> OK in {t_dur:.1f}s", flush=True)
    else:
        fail += 1
        err = result.stderr[-300:] if result.stderr else result.stdout[-300:]
        print(f"  -> FAILED in {t_dur:.1f}s: {err}", flush=True)
